...
def get_board_of_directors(ticker: str) -> str:
    ...
    if not data:
        return f"ℹ️ No board members found for {ticker}."

    output = f"Board of Directors for {ticker}:\n\n"
    for entry in data:
        for director in entry.get("directors", []):
            output += f"• {director.get('name', 'Unknown')} - {director.get('position', 'N/A')}\n"
    return output  # ← this is important: return a plain string
